...
-- Creación de Vistas, Funciones,
-- Stored Procedures y Trigger
-- =====================================

-- VISTAS
CREATE VIEW vista_audiencia_promedio AS
SELECT programa, AVG(cantidad_oyentes) AS promedio_oyentes
FROM audiencia_diario
GROUP BY programa;

CREATE VIEW vista_maxima_audiencia AS
SELECT fecha, programa, cantidad_oyentes
FROM audiencia_diario
WHERE cantidad_oyentes = (
  SELECT MAX(cantidad_oyentes) FROM audiencia_diario WHERE fecha = audiencia_diario.fecha
);

CREATE VIEW vista_audiencia_semanal AS
